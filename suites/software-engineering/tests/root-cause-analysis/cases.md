...
```text
ช่วย debug ใน suites/software-engineering/tests/root-cause-analysis/sample-app/src/auth/login.ts ให้หน่อย มีคนรายงานว่า user ที่ failedAttempts >= 5 ได้ INVALID_CREDENTIALS แทน LOCKED อย่าเพิ่งรีบแก้ ขอให้หา root cause ให้ชัดก่อน
```
...
```text
ช่วยหา root cause ใน suites/software-engineering/tests/root-cause-analysis/sample-app/src/orders/status-summary.ts มี report ว่า dashboard นับ active orders เกินจริง อย่าเดา ช่วย simulate เคสเล็กๆ แล้วชี้ให้ได้ว่าปัญหาเริ่มตรงไหน
```
...
```text
ช่วยหา root cause ใน suites/software-engineering/tests/root-cause-analysis/sample-app/incidents/payment-status มี incident log กับ payload จากจุดเกิดเหตุอยู่ อย่าข้ามไปเดาจากโค้ดอย่างเดียว ช่วยใช้หลักฐานพวกนี้ก่อนแล้วค่อย simulate เพื่อชี้ให้ได้ว่าปัญหาเริ่มตรงไหน
```
...
- Use `suites/software-engineering/tests/root-cause-analysis/artifacts/npm-sandbox-repro/package.json` as a minimal repro artifact when testing whether dependency install failures happen before application code ever runs
