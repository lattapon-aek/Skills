...
## Case 6: Compound (all five)

Request:

```text
ใช้ verification-hazards ทำ full hazard scan: fix สำหรับ bug ที่ผมยังไม่เคยเห็นเกิดจริง, unit test เขียวโดยเรียก handler ตรง, รันใน sandbox subset, sub-agent commit ให้, และ CI มี test อื่นแดงที่ผมไม่ได้แตะ.
```

Expected behavior:

- Produce a `Hazard Scan` marking 1–5 all `at risk` with the tell for each.
